...
public class MemberInfoDAC
{
    ...
    void FillParameters(SqlCommand command, MemberInfo item)
    {
    ...
    public void Update(MemberInfo item)
    {
        ...
        SqlCommand command= new SqlCommand(txt, _sqlCon);
        FillParameters(cmd, item);
        cmd.ExecuteNonQuery();
    }
    ...
    public MemberInfo[] SelectAll()
    {
        ...
        return list.ToArray(tyoepf(MemberInfo)) as MemberInfo[];
    }
}
